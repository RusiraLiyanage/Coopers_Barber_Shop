...
// Access-token lifetime fallback used when ACCESS_TOKEN_TTL_SECONDS is unset.
// The gateway uses the same value for the access cookie's max-age so the cookie
// can never outlive the token it carries.
export const DEFAULT_ACCESS_TOKEN_TTL_SECONDS = 120;
